...
import { useComponentI18n } from 'src/core/composables/useComponentI18n';
...
const r = {
  ...
  emailInput: ref<InstanceType<typeof QInput> | null>(null),
...
const isSaveDisabled = computed(() => {
  ...
  if (isNewUser) {
    ...
    return !hasChanges.value || !(isBaseValid && isPasswordOkOrEmpty);
  }
});
...
const m = {
  ...
  onEditUser: async (uIndex: number) => {
    ...
    if (r.emailInput.value) {
      await r.emailInput.value.validate();
    }
  },
  onCancelEditing: () => {
